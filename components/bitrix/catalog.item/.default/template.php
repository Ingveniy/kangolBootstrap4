...
{
    foreach ($arResult['ITEM']['JS_OFFERS'] as &$js_offer) {
        $js_offer['HEAD_PHOTO_COUNT'] = 0;
        foreach ($js_offer['MORE_PHOTO'] as $key => &$photo) {
            $desc = CFile::GetFileArray($photo["ID"]);
            $photo['DESC'] = $desc["DESCRIPTION"];

            $renderImage = CFile::ResizeImageGet($photo["ID"], Array("width" => 500, "height" => 500),BX_RESIZE_IMAGE_PROPORTIONAL_ALT, $jpgQuality = 100, false);
            $photo['SRC'] = $renderImage['src'];
        }
		$sort = array();
		...
		if ($_REQUEST['HEAD'] == 'Y') array_multisort($sort, SORT_DESC, $js_offer['MORE_PHOTO']);
		if ($_REQUEST['HEAD'] != 'Y') array_multisort($sort, SORT_ASC, $js_offer['MORE_PHOTO']);
    }
