		...
		switch($arItem['SETTINGS']['VIEW'])
		{
			...
			?>
				<div class="kombox-num kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					...
					<input 
						...
						size="5" 
						placeholder="<?echo $arItem["VALUES"]["MIN"]["VALUE"] + 0?>" 
					/>
					...
					<input 
						...
						size="5" 
						placeholder="<?echo $arItem["VALUES"]["MAX"]["VALUE"] + 0?>" 
					/>
				...
				<div class="kombox-select kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CLOSED"]):?> style="display:none;"<?endif;?>>
					...
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<option 
							...
							class="item_value_<?echo $ar["HTML_VALUE_ALT"]?> <?echo $ar["DISABLED"]? 'kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>"
						>
							<?echo $ar["VALUE"];?>
			...
			?>
				<div class="kombox-combo kombox-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CLOSED"]):?> style="display:none;"<?endif;?>>
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?php if($arItem["ID"] == 396 && !in_array($val,array(7432,7428,7433))) continue;?>
						...
						<div class="lvl2<?echo $ar["DISABLED"]? ' kombox-disabled': ''?><?echo $ar["CHECKED"]? ' kombox-checked': ''?>">
							...
							/>
							<label for="<?echo $ar["CONTROL_ID"]?>"><?echo (trim($ar["VALUE"]) == 'Великобритания' ? 'Англия' : $ar["VALUE"]);?> <span class="kombox-cnt">(<?echo $ar["CNT"];?>)</span></label>
						</div>
					<?endforeach;?>
