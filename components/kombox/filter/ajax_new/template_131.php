	...
	<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get"<?if($arResult['IS_SEF']):?> data-sef="yes"<?endif;?>>
		...
		foreach($arResult["ITEMS"] as $arItem):
			...
			<?if($showProperty):?>
				...
				<li class="w-col w-col-3 w-col-stack lvl1<?if($arItem["CLOSED"]):?> kombox-closed<?endif;?> props_<?=$arItem["ID"];?>" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
					<div class="kombox-filter-property-head">
						<i class="kombox-filter-property-i"></i>
						<span class="kombox-filter-property-name"><?echo ($arItem["NAME"] == 'форма головного убора' ? 'силуэт' : $arItem["NAME"])?></span>
